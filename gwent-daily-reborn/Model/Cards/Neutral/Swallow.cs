...
namespace gwent_daily_reborn.Model.Cards.Neutral
{
    internal class Swallow : Card
    {
        public Swallow(int x, int y, int width, int height, float confidence, int value = 6)
            : base(x, y, width, height, confidence, "Swallow", value)
        {
        }
        ...
        internal override bool Play(out ICollection<IBotTask> tasks, Rows xyType = Rows.Melee, Point placePoint = default)
        {
            ...
            return SelectCard(tasks) && RegisterCard() && Choose(tasks) && EndPlay(tasks);
        }
